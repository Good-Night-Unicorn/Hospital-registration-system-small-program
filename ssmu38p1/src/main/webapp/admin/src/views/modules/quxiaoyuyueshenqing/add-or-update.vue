<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="所属科室" prop="suoshukeshi">
          <el-input v-model="ruleForm.suoshukeshi" 
              placeholder="所属科室" clearable  :readonly="ro.suoshukeshi"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="医生简介" prop="yishengjianjie">
          <el-input v-model="ruleForm.yishengjianjie" 
              placeholder="医生简介" clearable  :readonly="ro.yishengjianjie"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="医生简介" prop="yishengjianjie">
              <el-input v-model="ruleForm.yishengjianjie" 
                placeholder="医生简介" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `quxiaoyuyueshenqing/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.quxiaoyuanyin = this.ruleForm.quxiaoyuanyin.replace(reg,'../../../ssmu38p1/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `quxiaoyuyueshenqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.quxiaoyuyueshenqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
