<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(68, 68, 68, 1)","textAlign":"center"}'>科室：</view>
					<view class="text" :style='{"color":"rgba(68, 68, 68, 1)","textAlign":"right","fontSize":"28rpx"}'>{{detail.keshi}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','keshi');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','keshi');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('keshi', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discusskeshi/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('keshi', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('keshi', this.detail);
				this.$utils.msg('审核成功');
